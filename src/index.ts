...
const sdk = getSdk(
  new GraphQLClient('https://api.cloudflare.com/client/v4/graphql', {
...
await Promise.allSettled([
  new Promise((res, rej) => {
    queryApi.queryRows(`${prefixQuery} |> filter(fn: (r) => r._field == "requests_total") ${suffixQuery}`, {
      next: (row, meta) => {
      ...
      complete() {
        res(log(`Last request time: ${requestTime.toString()}`));
      },
    });
  }),
  new Promise((res, rej) => {
    queryApi.queryRows(`${prefixQuery} |> filter(fn: (r) => r._field == "requests") ${suffixQuery}`, {
      next: (row, meta) => {
        const time = row[meta.column('_time').index];
  ...
  new Promise((res, rej) => {
    queryApi.queryRows(
      `${prefixQuery} |> filter(fn: (r) => r._field == "firewall_events_count") ${suffixQuery}`,
      {
        ...
        complete() {
          res(log(`Last firewall event time: ${firewallTime.toString()}`));
        },
      }
    );
  }),
  new Promise((res, rej) => {
    queryApi.queryRows(`${prefixQuery} |> filter(fn: (r) => r._field == "health_check_rtt") ${suffixQuery}`, {
      next: (row, meta) => {
        const time = row[meta.column('_time').index];
      ...
      complete() {
        res(log(`Last health event time: ${healthTime.toString()}`));
      },
    });
  }),
  new Promise((res, rej) => {
    queryApi.queryRows(
      `${prefixQuery} |> filter(fn: (r) => r._field == "worker_requests_count") ${suffixQuery}`,
      {
        ...
        complete() {
          res(log(`Last worker request time: ${workerTime.toString()}`));
        },
      }
    );
  }),
  new Promise((res, rej) => {
    queryApi.queryRows(
      `${prefixQuery} |> filter(fn: (r) => r._field == "r2_storage_payload_size") ${suffixQuery}`,
      {
        ...
        complete() {
          res(log(`Last r2 storage request time: ${r2StorageTime.toString()}`));
        },
      }
    );
  }),
  new Promise((res, rej) => {
    queryApi.queryRows(
      `${prefixQuery} |> filter(fn: (r) => r._field == "r2_operation_request") ${suffixQuery}`,
      {
        ...
        complete() {
          res(log(`Last r2 operation request time: ${r2OperationTime.toString()}`));
        },
      }
    );
  }),
...
async function doTask() {
  try {
    ...
    if (DateTime.now().diff(shouldFetchTime, 'minutes').minutes > 30)
      shouldFetchTime = shouldFetchTime.plus({
        minutes: DateTime.now().diff(shouldFetchTime, 'minutes').minutes - 30,
      });

    const zone = await sdk
      .fetchZoneAnalytics({
